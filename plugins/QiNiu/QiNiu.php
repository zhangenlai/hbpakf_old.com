...
require Env::get('root_path'). 'extend/getid3/getid3.php';
...
class QiNiu extends Plugin
{
    ...
    public function uploadAttachment($params = [])
    {
        ...
        //传的是音频或者视频文件 （需要获取时长信息）
        $shichang = '';
        if ($info->getExtension() == 'mp3' || $info->getExtension() == 'mp4'){

            $getID3 = new \getID3();  //实例化类
            $ThisFileInfo = $getID3->analyze($filePath); //分析文件，$path为音频文件的地址
            $shichang=$ThisFileInfo['playtime_string']; //这个获得的便是音频文件的时长
        }

...
//        $res = $app->customer_service_session->create('kf2001@xuanhuaShop', 'okp3m1AEBQGNijjqgVpqOsuc6A60');
        ...
        return ZBuilder::make('table')
            ...
            ->addColumns([ // 批量添加列
                ['id', 'ID'],
                ['name', '素材名'],
                ['type', '类型','','',['text' => '文字内容', 'image' => '图片素材', 'voice' => '声音素材', 'video' => '视频素材', 'thumb' => '缩略图素材', 'article' => '图文(文章)素材', 'news' => '图文(外链)素材']],
                ['media_id', 'media_id'],
                ['create_time', '上传日期'],
                ['right_button', '操作', 'btn']
            ])
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            switch ($post['type']) {
                case 'image':
                    $result = $this->wechatApp->material->uploadImage($root_path . get_file_path($post['image_file']));
                    $data['media_id'] = $result['media_id'];
                    $data['url'] = $result['url'];
                    $data['attachment_id'] = $post['image_file'];
                    break;
                case 'voice':
                    $result = $this->wechatApp->material->uploadVoice($root_path . get_file_path($post['voice_file']));
                    $data['media_id'] = $result['media_id'];
                    $data['attachment_id'] = $post['voice_file'];
                    break;
                case 'video':
                    $result = $this->wechatApp->material->uploadVideo($root_path . get_file_path($post['video_file']), $post['video_title'], $post['video_description']);
                    $data['media_id'] = $result['media_id'];
                ...
                case 'article':// 上传单篇文章，这种素材实际是微信公众平台上的图文素材。但不是微信文档中被动回复里的“图文消息”
                    $result_thumb = $this->wechatApp->material->uploadThumb($root_path . get_file_path($post['article_cover_pic']));
                    $article_data = [
                        'title' => $post['article_title'],
                        'thumb_media_id' => $result_thumb['media_id'],
                        'author' => $post['article_author'],
                        'digest' => $post['article_digest'],
                        'show_cover' => isset($post['article_show_cover_pic']) ? 1 : 0,
            ...
            if ($data['media_id'] || $post['type'] == 'news') {
                if (WeMaterialModel::create($data)) {
                    return $this->success('发布成功', null,'_parent_reload');
                } else {
...
EOF;
        return ZBuilder::make('form')
            ...
            ->addFormItems([ // 批量添加表单项
                ...
                ['file', 'voice_file', '声音上传', '支持mp3/wma/wav/amr格式,大小限制2M,不超过60秒', '', '2048', 'mp3,wma,wav,amr'],
                ...
                ['image', 'article_cover_pic', '文章封面', '支持bmp/png/jpeg/gif/jpg格式,大小限制2M', '', '2048', 'bmp,png,jpeg,gif,jpg'],
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($post['type'] == 'news') { // 修改单篇图文
                ...
                $data['name'] = $post['name'];
                $data['content'] = json_encode($news_data);
                $model = WeMaterialModel::get($post['id']);
                if ($model->save($data)) {
                ...
                $article = new Article($article_data);
                $result_article = $this->wechatApp->material->updateArticle($we_material_info['media_id'], $article, 0);
                $data['name'] = $post['name'];
                $data['attachment_id'] = $post['article_cover_pic'];
                $data['content'] = json_encode($article_data);
...
EOF;
    ...
    public function delete($ids = '')
    {
        ...
        foreach ($ids as $id) {
            if ($we_reply = WeReplyModel::where(['content' => $id])->field('keyword')->find()) {
                $this->error('删除失败: 自动回复中( 关键词: ' . $we_reply['keyword'] . ' )包含此素材( ID: ' . $id . ' )', cookie('__forward__'));
            }
            $info = WeMaterialModel::get($id);
            $result = $this->wechatApp->material->delete($info->media_id);
    ...
    public function uploadArticleImage($path = '')
    {
        ...
        $root_path = $_SERVER['DOCUMENT_ROOT'];
        $result = $this->wechatApp->material->uploadArticleImage($root_path . $path);
